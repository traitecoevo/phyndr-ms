...
To make this assessment new tools can be made including \textsc{opentree} synthetic tree \citep{OpenTree}.  For the specific examples in this project we also built a tool that dynamically builds a genus-family-order lookup for vascular plants from three canonical sources--the plant list, ApWeb, and Tank et al. higher taxa lookup.  This will keep the taxonomy resource up-to-date as systematic information improves through time.  It is freely available as the \textsc{TaxonLookup} R package at CRAN and at https://github.com/wcornwell/TaxonLookup ; currently only vascular plants are covered by this resource, but it could be extended to cover other taxa.  
